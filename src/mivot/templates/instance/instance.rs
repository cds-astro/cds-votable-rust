...
/// The same as `INSTANCE` **child of** `GLOBALS`, but having a `dmrole`.
...
impl Instance {
  ...
  pub fn visit<V: VodmlVisitor>(&mut self, visitor: &mut V) -> Result<(), V::E> {
    visitor.visit_instance_childof_instance_in_templates_start(self)?;
    ...
    visitor.visit_instance_childof_instance_in_templates_ended(self)
  }
}
      ...
      .and_then(|instance| {
        if instance.dmrole.is_empty() || instance.dmtype.is_empty()  {
          Err(VOTableError::Custom(format!(
            "Attributes 'dmrole' and 'dmtype' are mandatory and must be non-empty in tag '{}' child of INSTANCE child of TEMPLATES",
            Self::TAG
          )))
        } else {
          Ok(instance)
