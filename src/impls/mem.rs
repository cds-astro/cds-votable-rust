        ...
        loop {
            ...
            match &mut event {
                Event::Start(ref e) => match e.local_name() {
                    ...
                    b"TD" => {
                        let mut event =
                            reader.read_event(reader_buff).map_err(VOTableError::Read)?;
                        match &mut event {
                            Event::Text(e) => row
                                .push(e.unescape_and_decode(&reader).map_err(VOTableError::Read)?),
                            _ => eprintln!(
                                "Discarded event in {}: {:?}",
                                TableData::<Self>::TAG,
                                event
...
impl TableDataContent for InMemTableDataRows {
                ...
                Event::Start(ref e) => match e.local_name() {
                    ...
                    b"TD" => {
                        ...
                        match &mut event {
                            Event::Text(e) => {
                                let s =
                                    e.unescape_and_decode(&reader).map_err(VOTableError::Read)?;
    ...
    fn read_binary2_content<R: BufRead>(
        ...
        while let Ok(true) = binary_deser.has_data_left() {
            ...
            let null_flags: Vec<u8> =
                (&mut binary_deser).deserialize_tuple(n_bytes, bytes_visitor)?;
