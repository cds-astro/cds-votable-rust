...
use super::{
    data::Data, desc::Description, error::VOTableError, field::Field, group::TableGroup,
    info::Info, link::Link, param::Param, QuickXmlReadWrite, TableDataContent,
};
        ...
        loop {
            ...
            match &mut event {
                Event::Start(ref e) => match e.local_name() {
                    ...
                    Field::TAG_BYTES => self.elems.push(TableElem::Field(
                        from_event_start_by_ref!(Field, reader, reader_buff, e),
                    )),
                    Param::TAG_BYTES => self.elems.push(TableElem::Param(
                ...
                Event::Empty(ref e) => match e.local_name() {
                    Field::TAG_BYTES => self
                        .elems
                        .push(TableElem::Field(Field::from_event_empty(e)?)),
                    Param::TAG_BYTES => self
                        .elems
                        .push(TableElem::Param(Param::from_event_empty(e)?)),
                    TableGroup::TAG_BYTES => self
                        .elems
                        .push(TableElem::TableGroup(TableGroup::from_event_empty(e)?)),
...
impl<C: TableDataContent> QuickXmlReadWrite for Table<C> {
    ...
    fn read_sub_elements<R: BufRead>(
        ...
        self.read_sub_elements_by_ref(&mut reader, reader_buff, context)
            .map(|()| reader)
    }
