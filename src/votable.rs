        ...
        loop {
            ...
            match &mut event {
                ...
                Event::Start(ref mut e) if e.local_name() == VOTable::<C>::TAG_BYTES => {
                    let mut votable = VOTable::<C>::from_attributes(e.attributes())?;
                    let (resource, reader) =
                        votable.read_till_next_resource(reader, &mut reader_buff)?;
                    reader_buff.clear();
        ...
        loop {
            ...
            match &mut event {
                Event::Start(ref e) => match e.local_name() {
                    ...
                    Info::TAG_BYTES if self.resources.is_empty() => self.elems.push(
                        VOTableElem::Info(from_event_start_by_ref!(Info, reader, reader_buff, e)),
                    ),
                    Group::TAG_BYTES => self.elems.push(VOTableElem::Group(
                        from_event_start_by_ref!(Group, reader, reader_buff, e),
                    )),
                    Param::TAG_BYTES => self.elems.push(VOTableElem::Param(
                        from_event_start_by_ref!(Param, reader, reader_buff, e),
                    )),
        ...
        loop {
            ...
            match &mut event {
                Event::Start(ref e) => match e.local_name() {
                    ...
                    Info::TAG_BYTES if self.resources.is_empty() => self.elems.push(
                        VOTableElem::Info(from_event_start!(Info, reader, reader_buff, e)),
                    ),
                    ...
                    Definitions::TAG_BYTES => self.elems.push(VOTableElem::Definitions(
                        from_event_start!(Definitions, reader, reader_buff, e),
                    )),
                    Resource::<C>::TAG_BYTES => {
                        let resource = Resource::<C>::from_attributes(e.attributes())?;
    ...
    fn from_attributes(attrs: Attributes) -> Result<Self, VOTableError> {
        ...
        for attr_res in attrs {
            ...
            votable = match attr.key {
                ...
                b"version" => {
                    votable.set_version(Version::from_str(value).map_err(VOTableError::Custom)?)
                }
  ...
  xsi:schemaLocation="http://www.ivoa.net/xml/VOTable/v1.3 http://www.ivoa.net/xml/VOTable/v1.3">
   ...
   &lt;TD&gt;&lt;/TD&gt;, explicitely possible from VOTable-1.3
    ...
    fn test_votable_read_datatable_from_file() {
        ...
        let votable =
            VOTableWrapper::<InMemTableDataRows>::from_ivoa_xml_file("resources/sdss12.vot")
                .unwrap()
                .unwrap();
    ...
    fn test_votable_read_obscore_file() {
        let votable =
            VOTableWrapper::<InMemTableDataRows>::from_ivoa_xml_file("resources/obscore.vot");
        assert!(votable.is_ok())
    }
    ...
    fn test_votable_read_binary_from_file() {
        let votable =
            VOTableWrapper::<InMemTableDataRows>::from_ivoa_xml_file("resources/binary.b64")
                .unwrap()
                .unwrap();
        match toml::ser::to_string_pretty(&votable) {
            Ok(_content) => println!("\nOK"), // println!("{}", &content),
